...
function uplode_files($data,&$to){
    $is_upl = false;
        ...
        for($i = 0, $count = count($data['tmp_name']); $i < $count; $i++){
            ...
            if(is_uploaded_file($data['tmp_name'][$filename])){
                    //проверяем загружено ли изображение(файл на нужный нам тип)
                    $file_type = isTypeValide($data['name'][$filename]);
                    if($file_type){
                        //файл успешно загружен, перемещаем его в текущий каталог
                        $new_file_name = "gallery/"."pic".(++$len).".".$file_type;
                         if(move_uploaded_file($data['tmp_name'][$filename], $new_file_name )){
                        ...
                        echo "Файл ".$data['name'][$filename]." недопустимого типа<br>";
            }
        }
        return $is_upl;
...
if($_FILES){
    if(isFormValid()){
        ...
        redirect("6.php?flash={$flashMessage}");
    }
}
    ...
    <body>
        <div id="container">
            ...
            <form method="post" enctype="multipart/form-data">
                <input type="file" name="data[filename1]"/><br>
                <input type="file" name="data[filename2]"/><br>
                <input type="file" name="data[filename3]"/><br>
                <input type="file" name="data[filename4]"/><br>
                <input type="file" name="data[filename5]"/><br>
